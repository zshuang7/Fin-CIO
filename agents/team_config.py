...
wall_street_agent = Agent(
    ...
    instructions=[
        ...
        "  Phase 1 — STRUCTURED SIGNALS (required):",
        "    Call: get_wall_street_signals(ticker)",
        "    → Returns JSON with: consensus distribution + extracted bank mentions + vote tally.",
        "",
        "  Phase 2 — DEEP CONTEXT (conditional):",
        "    Call: wall_street_search(ticker, '<bank/theme>') ONLY IF:",
        "      • bank_votes < 3, OR",
        "      • user explicitly asks for a specific bank/report, OR",
        "      • the headline signals are ambiguous and need 'why'.",
        "",
        "OUTPUT MUST INCLUDE THIS SECTION (MANDATORY, DO NOT SKIP):",
        "  ## 🗳️ Wall Street Conviction Board",
        "     - Vote tally (banks as voters; newest signal per bank):",
        "       🐂 Bullish  ██████████░░░  N banks",
        "       😐 Neutral  ████░░░░░░░░░  N banks",
        "       🐻 Bearish  ██░░░░░░░░░░░  N banks",
        "     - Latest key calls (newest first, 4-8 lines):",
        "       🟢 <Bank> · <Date> · <Rating> · PT <X>  → <1-line idea>",
        "       ⚪ <Bank> · <Date> · <Rating> · PT <X>  → <1-line idea>",
        "       🔴 <Bank> · <Date> · <Rating> · PT <X>  → <1-line idea>",
        "",
        "Then add (short):",
        "  ## 💡 Street Key Ideas (Latest)",
        "  - 3 bullets: what bulls cite, what bears cite, what the 'swing factor' is.",
        "",
        "  ## ⚠️ Conflicts & Freshness",
        "  - Are bank call dates newer than financial data date? Flag DATA GAP if so.",
        "",
        "Hard rules:",
        "  - If you cannot find bank-level calls: still print the Board section with",
        "    N/A for bank lines, but keep the vote tally from consensus distribution.",
...
cio_team = Team(
    ...
    instructions=[
        ...
        "    Output: Standard sections INCLUDING ## 🗳️ Wall Street Conviction Board (mandatory).",
        "",
        ...
        "    ★ Wall Street Conviction Board: MUST come from WallStreetAgent signals.",
        "",
        "  STOCK_ANALYSIS — Level 4-5:",
        "    MacroAgent → CompanyAgent → WallStreetAgent → NewsAgent → YOUR synthesis",
        "    ★ Wall Street Conviction Board: MUST include dated bank calls + voting.",
        "",
        "  MARKET_ANALYSIS:",
